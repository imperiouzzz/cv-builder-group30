...
export default function FinishScreen({ onEdit, onExport }: Props) {
  const { cv, breakdown, resetCV } = useCVStore();
  ...
  if (score < 70 && cv.workExp.some(w => !w.description))
    tips.push('Add bullet point achievements to your work experience entries.');
  ...
  return (
    <div style={{
      ...
      <h2 style={{ fontFamily: 'Fraunces, serif', fontSize: 26, fontWeight: 600, color: '#1A1A2E', marginBottom: 10 }}>
        Your CV is Complete!
      </h2>
      <p style={{ fontSize: 14, color: '#718096', maxWidth: 440, lineHeight: 1.6, marginBottom: 28 }}>
        Great work, <strong>{cv.fullName || 'there'}</strong>! Review the live preview on the right,
        then export your ATS-optimised PDF.
      </p>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'left' }}>
          <div style={{ fontSize: 11, color: '#718096' }}>ATS Score out of 100</div>
      ...
      {tips.length > 0 && (
        <div style={{
          background: '#FFFBF0', border: '1px solid #FAD56A',
          borderRadius: 10, padding: '14px 18px', marginBottom: 24,
          maxWidth: 440, textAlign: 'left',
        }}>
          <p style={{ fontSize: 11, fontWeight: 600, color: '#744210', marginBottom: 8 }}>
            💡 Tips to improve your score:
          </p>
          <ul style={{ listStyle: 'none', fontSize: 12, color: '#7B5E17' }}>
            {tips.map((t, i) => <li key={i} style={{ padding: '2px 0' }}>• {t}</li>)}
          </ul>
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center' }}>
        <ActionBtn primary onClick={onExport}>📥 Download PDF</ActionBtn>
        <ActionBtn onClick={handleOpenInTab}>🔗 Open in Tab</ActionBtn>
        <ActionBtn onClick={handlePrint}>🖨 Print CV</ActionBtn>
        <ActionBtn onClick={onEdit}>✏ Edit CV</ActionBtn>
      ...
      <p style={{ fontSize: 11, color: '#718096', marginTop: 18, maxWidth: 380, lineHeight: 1.6 }}>
        <strong>Download PDF</strong> saves the file ·{' '}
        <strong>Open in Tab</strong> lets you view &amp; print via your browser ·{' '}
        <strong>Print CV</strong> opens your printer directly
      </p>
    </div>
  );
}
...
const PRINT_FONTS: Record<string, { bodyFamily: string; nameFamily: string; googleUrl: string }> = {
  sans: {
...
function buildPrintHTML(cv: CVData): string {
  ...
  const e = (s: string | undefined | null) =>
    String(s || '')
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
  ...
  const renderSection: Record<string, () => string> = {
    summary:      () => cv.summary
      ? sec('Professional Summary', `<p class="summary">${e(cv.summary)}</p>`)
      : '',
    education:    () => cv.education?.length
      ? sec('Education', cv.education.map(ed => `
          <div class="entry">
      ...
      : '',
    work:         () => cv.workExp?.length
      ? sec('Work Experience', cv.workExp.map(w => `
          <div class="entry">
    ...
    projects:     () => cv.projects?.length
      ? sec('Projects &amp; Research', cv.projects.map(p => `
          <div class="entry">
      ...
      : '',
    volunteering: () => cv.volunteering?.length
      ? sec('Volunteering', cv.volunteering.map(v => `
          <div class="entry">
    ...
    skills:       () => cv.skills?.length
      ? sec('Skills', `<div class="skills">${cv.skills.map(s=>`<span class="pill">${e(s.name)}</span>`).join('')}</div>`)
      : '',
    references:   () => cv.references?.length
      ? sec('References', cv.references.map(r => `
          <div class="entry">
      ...
      : '',
    custom:       () => cv.customSections?.filter(c=>c.sectionTitle).map(c =>
      sec(e(c.sectionTitle), `<div class="detail">${e(c.entries)}</div>`)).join('') ?? '',
  };
  ...
  if (Array.isArray(rawOrder) && rawOrder.length > 0) {
    ...
    try { const p = JSON.parse(rawOrder); if (Array.isArray(p)) order = p; } catch (_) {}
  }
  if (!order.length) {

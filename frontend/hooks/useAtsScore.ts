...
const ACTION_VERBS = new Set([
  ...
  'enhanced','established','executed','facilitated','generated','grew',
  'implemented','improved','increased','initiated','integrated','launched',
  'led','maintained','managed','mentored','migrated','optimised','optimized',
  'organised','organized','oversaw','performed','planned','produced','provided',
  'reduced','refactored','resolved','reviewed','scaled','secured',
  'spearheaded','streamlined','supervised','tested','trained','transformed',
...
export function computeLocalScore(cv: ReturnType<typeof useCVStore.getState>['cv']): AtsBreakdown {
  let content = 0, ats = 0, section = 0, impact = 0, appReady = 0;
  ...
  const allDesc = [
    ...cv.workExp.map(w => w.description),
  ...
  const total = Math.min(
    Math.min(content, 35) + Math.min(ats, 25) + Math.min(section, 20) +
    Math.min(impact, 15)  + Math.min(appReady, 5),
    100
  );
...
export function useAtsScore() {
  const { cv, setBreakdown } = useCVStore();

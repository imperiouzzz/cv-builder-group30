...
import type { CVData, EducationEntry, WorkEntry, Skill, ProjectEntry, VolunteeringEntry, ReferenceEntry } from '@/types/cv.types';
import { emptyCV } from '@/store/cvStore';

// ── Section heading keyword map ──────────────────────────────
const SECTION_MAP: Array<{ key: string; re: RegExp }> = [
  { key: 'summary',      re: /\b(summary|profile|objective|about me|personal statement|career objective)\b/i },
  { key: 'education',    re: /\b(education|academic|qualifications?|degrees?|schooling)\b/i },
  { key: 'work',         re: /\b(work experience|work history|employment|professional experience|internship experience)\b|^experience$/i },
  { key: 'leadership',   re: /\b(leadership)\b/i },
  { key: 'volunteering', re: /\b(volunteer|voluntary|extra.?curricular|extracurricular|community|activities)\b/i },
  { key: 'skills',       re: /\b(skills?|abilities|competenc|expertise|technologies|proficiencies)\b/i },
  { key: 'projects',     re: /\b(projects?|research|portfolio)\b/i },
  { key: 'references',   re: /\b(references?|referees?)\b/i },
  { key: 'custom',       re: /\b(awards?|certifications?|publications?|achievements?|honours?|hobbies|interests?|memberships?)\b/i },
];
...
function classifyHeading(line: string): string | null {
  ...
  const isTitleish = /^[A-Z][a-zA-Z\s\/\-&]{2,}$/.test(line)
    && !/[,\.\!\?;:]/.test(line)
...
function extractDates(block: string): { start: string; end: string } {
  const r = DATE_RANGE_RE.exec(block);
  if (r) return { start: toTitleDate(r[1]), end: /^(present|current|now)$/i.test(r[2]) ? 'Present' : toTitleDate(r[2]) };
  const y = YEAR_RANGE_RE.exec(block);
  if (y) return { start: y[1], end: /^(present|current|now)$/i.test(y[2]) ? 'Present' : y[2] };
  const m = SINGLE_MONTH_RE.exec(block);
  if (m) return { start: toTitleDate(m[1]), end: '' };
  const s = SINGLE_YEAR_RE.exec(block);
  if (s) return { start: s[1], end: '' };
  return { start: '', end: '' };
}

// ── String helpers ───────────────────────────────────────────
function toTitleDate(s: string) { return s.replace(/\b\w/g, c => c.toUpperCase()); }
...
// ── Split helpers ────────────────────────────────────────────
function splitByPredicate(lines: string[], pred: (l: string, i: number) => boolean): string[][] {
  if (!lines.length) return [];
  const result: string[][] = [];
  let buf: string[] = [lines[0]];
  for (let i = 1; i < lines.length; i++) {
    if (pred(lines[i], i)) { if (buf.length) result.push([...buf]); buf = []; }
    buf.push(lines[i]);
  }
...
function splitByBlankLines(lines: string[]): string[][] {
  ...
  let buf: string[] = [];
  lines.forEach(l => {
    if (!l.length && buf.length) { result.push([...buf]); buf = []; }
...
export function parseCV(raw: string): ParseResult {
  const cv = emptyCV();
  ...
  for (const line of lines) {
    ...
    if (h !== null) { curSec = h; sections[curSec] = sections[curSec] || []; }
    else { sections[curSec] = sections[curSec] || []; sections[curSec].push(line); }
  }
  ...
  if (sections.education) {
    ...
    entries.forEach(entry => {
      ...
      const block = nb.join('\n');
      const edu: EducationEntry = { id: uid(), degree: '', institution: '', location: '', startDate: '', endDate: '', gpa: '', achievements: '' };
      ...
      if (/expected\s+to\s+(graduate|finish|complete)/i.test(block)) {
        ...
        if (startLines.length) { const ds = extractDates(startLines.join('\n')); if (ds.start) edu.startDate = ds.start; }
      } else {
        const d = extractDates(block);
        edu.startDate = d.start; edu.endDate = d.end;
      }
  ...
  if (sections.work) {
    ...
    const isJobHeader = (l: string) =>
      (/\|/.test(l) && TITLE_WORDS.test(l)) ||
      (TITLE_WORDS.test(l) && l.length < 100 && !l.startsWith('•') && !l.startsWith('-') && !/^(key responsibilities)/i.test(l));
    ...
    entries.forEach(entry => {
      ...
      if (!nb.length) return;
      const block = nb.join('\n');
      const job: WorkEntry = { id: uid(), title: '', company: '', location: '', startDate: '', endDate: '', description: '' };
      ...
      if (/\|/.test(nb[0])) {
        ...
        if (m) { job.title = clean(m[1]); job.company = clean(m[2]); }
      } else {
      ...
      const d = extractDates(block);
      job.startDate = d.start; job.endDate = d.end;
      ...
      const bullets = nb
        .filter(l => /^[•\-*>–]/.test(l) || (l.length > 40 && !isJobHeader(l) && !DATE_ONLY.test(l) && !/^(key responsibilities|at bsystems|during my|this experience|this internship)/i.test(l)))
        .slice(0, 12)
        .map(l => l.replace(/^[•\-*>–]\s*/, '• '));
  ...
  if (sections.skills) {
    ...
    skillLines.forEach(line => {
      ...
      if (final.length >= 2 && /[a-zA-Z]/.test(final) && !cv.skills.find(s => s.name === final)) {
        cv.skills.push({ id: uid(), name: final, category: 'technical' });
      }
    });
    if (!cv.skills.length) {
      skillLines.join(', ').split(/[,\n]+/).map(s => s.replace(/^[•\-*\s]+/, '').trim()).forEach(s => {
        if (s.length >= 2 && s.length <= 50 && /[a-zA-Z]/.test(s)) {
          cv.skills.push({ id: uid(), name: s, category: 'technical' });
        }
      });
    }
  }
  ...
  if (sections.projects) {
    ...
    splitByBlankLines(projLines).forEach(entry => {
      ...
      const proj: ProjectEntry = { id: uid(), name: '', tech: '', period: '', description: '', link: '' };
  ...
  if (volSrc.some(l => l.length > 3)) {
    ...
    splitByPredicate(volLines, (l, i) =>
      i > 0 && !l.startsWith('•') && !l.startsWith('-') && l.length > 3 && l.length < 100 &&
      !/^(key responsibilities)/i.test(l)
    ).forEach(entry => {
      const nb = entry.filter(l => l.length > 0);
      if (!nb.length) return;
      const block = nb.join('\n');
      const vol: VolunteeringEntry = { id: uid(), role: '', org: '', period: '', desc: '' };
  ...
  if (sections.references) {
    ...
    if (!/available on request|upon request/i.test(refLines.join(' '))) {
      ...
      function isPersonName(l: string): boolean {
      ...
      finalEntries.forEach(entry => {
        ...
        if (!nb.length) return;
        const block = nb.join('\n');
        const ref: ReferenceEntry = { id: uid(), name: '', title: '', org: '', email: '', phone: '' };
        ...
        if (pM) { const d = pM[1].replace(/\D/g, ''); if (d.length >= 7) ref.phone = pM[1].trim(); }
